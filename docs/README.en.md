...
These pages distinguish shipped behavior, platform limits, and roadmap items. Compatibility mode keeps the upstream default Web client below an independent Desktop frame; extended mode installs its own Desktop layout/sidebar registration and hosts official occupants in an inverted L; enhanced mode retains a separate root registration with compact internal captions. Desktop frames provide capability-gated native materials. The plugin marketplace is complete and built in through [`dsh-community-market`](../dsh-community-market/README.md), with catalog browsing and managed package operations inside the Desktop window; mobile remote control and Channels remain separate roadmap items and are not implied to be part of the current installer.
